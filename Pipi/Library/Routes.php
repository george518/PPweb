...
class Routes
{
	...
	/**
	 * [normalUrl 接口-正常url模式 按照module/controller/action/{id}]
	...
	protected function normalUrl()
	{
	    ...
	    $route  = []; 
	    $route['module']     = (isset($mca[0]) && $mca[0]) ? $mca[0] : C('MODULE');
	    $route['controller'] = isset($mca[1]) ? $mca[1] : C('CONTROLLER');
	    $route['action']     = isset($mca[2]) ? $mca[2] : C('ACTION');
	    #设置URL
